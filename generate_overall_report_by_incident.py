...
# -*- coding: utf-8 -*-
"""
Создает общий отчет по всем службам вместе с сопоставлением по номеру инцидента.
Структура: Сводка_количество, Матрица_жалоб, Детальные
"""
...
def main():
    ...
    print("=== Создание общего отчета ===")
    summary = build_summary_counts(df_112)
    matrix = build_region_complaint_matrix(df_112)
    region_status = build_region_status_stats(df_112)

    out_dir = Path('reports') / '2026-01_overall'
    out_dir.mkdir(parents=True, exist_ok=True)

    timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    out_xlsx = out_dir / f'ОБЩИЙ_ОТЧЕТ_ВСЕ_СЛУЖБЫ_{timestamp}.xlsx'
    ...
    print(f"✅ Общий отчет сохранен: {out_xlsx}")
    print(f"\nСтатистика:")
    print(f"  Всего записей: {len(df_112):,}")
    print(f"  С жалобами: {int(df_112['Есть_жалоба'].sum()):,}")
    print(f"  Регионов: {len(matrix)}")
