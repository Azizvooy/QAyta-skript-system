"""
Финальный анализ уникальных заявок
"""
import pandas as pd

print("=" * 80)
print("АНАЛИЗ УНИКАЛЬНЫХ ЗАЯВОК - ФИНАЛЬНАЯ ВЕРСИЯ")
print("=" * 80)
...
# Дедупликация: берем последнюю запись для каждой карты
# Поскольку нет надежной колонки с датой, используем просто последнюю запись
df_unique = df_clean.drop_duplicates(subset=номер_карты, keep='last')

print(f"\n🎫 УНИКАЛЬНЫХ ЗАЯВОК (КАРТ): {len(df_unique):,}")
print("=" * 80)
...
# ========== ТОП ОПЕРАТОРОВ ==========
print(f"\n" + "=" * 80)
print("👥 ТОП-15 ОПЕРАТОРОВ ПО КОЛИЧЕСТВУ УНИКАЛЬНЫХ ЗАЯВОК:")
print("=" * 80)

df_ops = df_unique[df_unique[оператор].notna() & (df_unique[оператор] != '-')]

if len(df_ops) > 0:
    ops_counts = df_ops[оператор].value_counts().head(15)
...
status_counts = df_unique[статус].value_counts().head(20)
...
output_file = r'c:\Users\a.djurayev\Desktop\QAyta skript\UNIQUE_CARDS_ANALYSIS.csv'
df_unique.to_csv(output_file, index=False, encoding='utf-8-sig')
print(f"\n💾 Уникальные карты сохранены в: {output_file}")
